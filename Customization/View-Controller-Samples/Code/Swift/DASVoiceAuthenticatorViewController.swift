...
class DASVoiceAuthenticatorViewController: DASAuthenticatorViewControllerBase,
    @MainActor DASVoiceControllerDelegate
{
     ...
     */
    lazy var voiceController: DASVoiceControllerProtocol =
        DASVoiceAuthenticatorFactory.createVoiceController(
            context: singleAuthenticatorContext, delegate: self)
    ...
    override func viewDidLoad() {
        ...
        self.title =
            "\(singleAuthenticatorContext?.authenticatorInfo?.authenticatorName ?? "Voice") (Swift)"
    ...
    func enableLevelMeter() {
        levelMeter.enableButtonMode(
            startText: localise("Voice Screen - Action - Start Recording"),
        ...
        if voiceController.isRecording() {
            ...
            AVCaptureDevice.requestAccess(for: AVMediaType.audio) { [self] granted in
                DispatchQueue.main.async {
                    if granted {
                        if let error = self.voiceController.startRecording() {
    ...
    func updateProgress() {
        if let label = self.progressLabel {
            label.text = String(
                format: localise("Voice Screen - Progress - Formatted"),
                voiceSampleIndex,
